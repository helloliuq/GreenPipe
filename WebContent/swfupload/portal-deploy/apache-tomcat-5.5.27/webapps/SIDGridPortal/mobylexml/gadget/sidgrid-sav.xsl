...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="xq:head">
    ...
    <div class="formHead">
      <span class="formTitle">
        <h1>
        	<xsl:value-of select="xq:doc/xq:title" />
		</h1>
        <!-- Description -->
        <h2>
      ...
      <span class="formCtrl">
        ...
        <xsl:choose>
          <xsl:when test="$isInPortal!='true'">
            <label >
              <xsl:attribute name="for">form_email_value<xsl:value-of select="xq:name" /></xsl:attribute>        
              E-mail:
            </label>
            <input name="email" type="text">
              <xsl:attribute name="id">form_email_value<xsl:value-of select="xq:name" /></xsl:attribute>
          ...
          <xsl:otherwise>
            <input name="email_mandatory" type="hidden">
              <xsl:attribute name="id">email_mandatory_<xsl:value-of select="xq:name" /></xsl:attribute>
        ...
        <input type="submit" value="Run">
          <xsl:attribute name="id">submit_<xsl:value-of select="xq:name" /></xsl:attribute>
        </input>
      </span>
      <span class="formCtrl">
  ...
  <xsl:template name="Databox">
    ...
    <legend>
      ...
      (<xsl:value-of select="normalize-space(concat(xq:type/xq:biotype, ' ', xq:type/xq:datatype/xq:class))" />)
      <xsl:call-template name="commentToggle">
        <xsl:with-param name="commentNode" select="xq:comment"/>
