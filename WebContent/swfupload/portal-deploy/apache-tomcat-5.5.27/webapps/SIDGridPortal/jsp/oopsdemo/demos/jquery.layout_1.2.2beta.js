 ...
 *
 * $Date: 2009-03-27 02:17:22 +0100 (sab, 27 dic 2008) $
 * $Rev: 203 $
 * 
 ...
 */
	...
	,	defaults: { // default options for 'all panes' - will be overridden by 'per-pane settings'
		...
		,	noRoomToOpenTip:		"Not enough room to open this pane."
		...
		,	cursorHotkeyModifier:	"CTRL+SHIFT" // either 'SHIFT', 'CTRL' or 'CTRL+SHIFT' - NOT 'ALT'
	...
	var config = {
		allPanes:		"south,north,east,west,center"
	,	borderPanes:	"south,north,east,west"
	,	zIndex: { // set z-index values here
			resizer_normal:	2		// normal z-index for resizer-bars
		,	pane_normal:	3		// normal z-index for panes - 'center' will be 1-less than this
 ...
 */
	...
	var execUserCallback = function (pane, v_fn) {
		...
		try {
			...
			if (typeof fn=="function")
				...
				return fn( pane, $Ps[pane], $.extend({ test: 'OK' },state[pane]), $.extend({},options[pane]), options.name );
		}
		catch (ex) {}
	};
	...
	var cssNum = function ($E, prop) {
		...
		,	visibility = ""
		;
		if (!$E.jquery) $E = $($E); // in case DOM element passed - NEW
	...
	var calcSize = function (pane, size, setAutoSize, dim) {
		var
			o	= options[pane]
		,	s	= state[pane]
		,	c	= config[pane]
		...
		if (typeof size == "string") {
			...
			else // string invalid if not %
				size = 0;
		}

		if (size >= 1) {
			v = Math.floor(Number(size));
			if (setAutoSize) s.autoSize = false;
		}
		else if (size <= 0) {
			v = 0;
			if (setAutoSize) s.autoSize = true;
		}
		else if (size > 0) { // fraction between 0 and 1
			if (!dim) dim = (c.dir == "horz" ? "innerHeight" : "innerWidth")
			v = max(Math.floor(cDims[dim] * size) - (o.spacing_open || 0), 0);
			if (setAutoSize) s.autoSize = true;
		}
		//if (setAutoSize) alert( 'calcSize \n'+ pane +'\n size = '+ size +'\n autoSize = '+ s.autoSize );
	...
	var setPaneMinMaxSizes = function (pane, slide) {
		var 
			d				= cDims
		,	o				= options[pane]
		,	s				= state[pane]
		...
		,	altS			= state[altPane]
		,	$altP			= $Ps[altPane]
		,	altPaneSize		= (!$altP || altS.isHidden || altS.isClosed ? 0 : (dir=="horz" ? $altP.outerHeight() : $altP.outerWidth()))
		,	altPaneSpacing	= (!$altP || altS.isHidden ? 0 : options[altPane][ altS.isClosed ? "spacing_closed" : "spacing_open" ])
		...
		,	limitSize		= containerSize - paneSpacing - (isSliding ? 0 : (minCenterSize + altPaneSize + altPaneSpacing))
		,	minSize			= calcSize(pane, o.minSize)
		,	maxSize			= min(calcSize(pane, o.maxSize) || 9999, limitSize)
		,	minPos, maxPos	// used to set resizing limits
		;
s.test = {};
s.test.containerSize = containerSize;
s.test.paneSpacing = paneSpacing;
s.test.altPaneSize = altPaneSize;
s.test.altPaneSpacing = altPaneSpacing;

		// set min/max 'position' (for resizing) - accounts for 'padding' inside the container
		switch (pane) {
	 ...
	 * confirmRoomToOpen
	 *
	 * Checks where a pane has room to open, and sets the pane accordingly - returns boolean
	 *
	 * @returns Boolean  Returns true if has room, else false
	 */
	var confirmRoomToOpen = function (pane, isOpening, isSliding) {
		var
			o	= options[pane]
		,	s	= state[pane]
		;
		setPaneMinMaxSizes(pane, isSliding); // update pane-state
		if (s.minSize > s.maxSize) { // pane cannot fit on screen!
			s.noRoom = true; // update state
			s.wasOpen = !s.isClosed && !s.isSliding;
			if (noRoomToOpenAction == "close")
				close(pane, true, true);
			else if (noRoomToOpenAction == "hide")
				hide(pane, true);
//alert( pane +'.confirmRoomToOpen = false' );
			return false;
		}
		else if (s.size > s.maxSize) // pane is too big - shrink it
		...
		if (s.noRoom) {
			s.noRoom = false; // update state
			if (isOpening)
				; // already opening - do nothing
			else if (o.noRoomToOpenAction == "close" && s.wasOpen)
				open(pane);
			else if (o.noRoomToOpenAction == "hide")
				show(pane, s.wasOpen);
		}
 ...
 */
	...
	var initPanes = function () {
		...
		$.each(c.allPanes.split(","), function() {
			...
			,	size	= calcSize(pane, o.size, true)
			,	minSize	= calcSize(pane, o.minSize) || 1
			,	maxSize	= calcSize(pane, o.maxSize) || 9999
			...
			s.elem = $P.attr("tagName").toUpperCase(); // eg: DIV, IFRAME, etc.
			if (pane != "center") {
			...
			switch (pane) {
				...
				case "east": 	CSS.right 	= cDims.right; // ditto
								break;
				case "center":	CSS.zIndex	= c.defaults.cssReq.zIndex - 1; // ensure borderPanes ALWAYS overlay center!
		...
		$.each(c.borderPanes.split(","), function() {
			...
			,	$P		= $Ps[pane]
			;
			if ($P && !s.isClosed && !s.isHidden) // pane is OPEN
				// pane may be Closed, Hidden or Resized by confirmRoomToOpen()
				confirmRoomToOpen(pane);
		});

//alert( 'initPanes \n\n CALL sizeMidPanes("center", null, true)' );
		// size center-pane AGAIN in case we 'closed' a border-pane in loop above
		sizeMidPanes("center", null, true); // true = onInit
//debugData( state.center, "sizeMidPanes( center )" )
	...
	var initHandles = function () {
		...
		$.each(c.borderPanes.split(","), function() {
			...
			var 
				edge	= c[pane].edge
			,	isOpen	= !s.isClosed // $P.is(":visible")
			...
			// if pane is 'hidden', then hide the Resizer-bar
			if (s.isHidden) $R.hide();
			...
			,	start: function (e, ui) {
					...
					$(o.maskIframesOnResize === true ? "iframe" : o.maskIframesOnResize).each(function() {					
						$('<div class="ui-layout-mask"/>')
							...
							.css($(this).position()) // top & left -- NEW: changed from offset()
 ...
 */
	 ...
	 */
	var hide = function (pane, noCallbacks) {
		var
			o	= options[pane]
		,	s	= state[pane]
		,	$P	= $Ps[pane]
		...
		,	isC	= (pane == "center")
		;
		if (!isC && (!$P || s.isHidden)) return; // pane does not exist OR is already hidden

		// onhide_start callback - will CANCEL hide if returns false
		if (!noCallbacks && false === execUserCallback(pane, o.onhide_start)) return;

		if (isC) {
			$P.hide();
			if (!noCallbacks) execUserCallback(pane, o.onhide_end || o.onhide);
			return;
		}
		...
		if (noCallbacks || s.isClosed) {
			s.isClosed = true; // to trigger open-animation on show()
			s.isHidden = true;
			$P.hide(); // no close animation
			sizeMidPanes(c[pane].dir == "horz" ? "all" : "center");
			if (!noCallbacks) execUserCallback(pane, o.onhide_end || o.onhide);
		}
		else {
			s.isHiding = true; // used by close() logic
			close(pane); // will hide pane after closing and adjust all panes to fit
			//s.isHidden  = true; - will be set by close - if not cancelled
		}
	};

	var show = function (pane, openPane, slide, noAnimation) {
		var
			o	= options[pane]
		,	s	= state[pane]
		,	$P	= $Ps[pane]
		...
		,	isC	= (pane == "center")
		;
		if (!isC && (!$P || !s.isHidden)) return; // pane does not exist OR is not hidden
		...
		if (openPane === false) // default = true
			close(pane, true); // true = force
		else
			open(pane, slide, noAnimation);
	};
	 ...
	 */
	var toggle = function (pane, slide, noAnimation) {
		var s = state[pane];
		if (s.isHidden)
	...
	var close = function (pane, force, noAnimation) {
		...
		// SUBROUTINE
		function close_2 () {
			bindStartSlidingEvent(pane, true); // will enable ONLY if state.PANE.isSliding = true
	...
	var open = function (pane, slide, noAnimation) {
		...
		if (s.isHidden && !isShowing) {
			show(pane, true, slide, noAnimation);
			return; // show will call open AGAIN - after making Resizer visible (isShowing = true)
		}

		// make sure there is enough space available to open the pane
		if ($P && !s.isClosed && !s.isHidden) { // pane is OPEN
			// pane may be Closed, Hidden or Resized by confirmRoomToOpen()
			if (!confirmRoomToOpen(pane, true, true)) {
				syncPinBtns(pane, false); // make sure pin buttons are reset
				if (o.noRoomToOpenTip) alert(o.noRoomToOpenTip);
//debugData( state[pane], "state."+pane ); // DEBUGGING
				return; // ABORT
			}
		}
		else s.noRoom = false;
		...
		s.isClosed = false;
		if (isShowing) s.isHidden = false; // update isHidden BEFORE sizing panes

		// 'PIN PANE' - stop sliding
		if (s.isSliding && !slide) // !slide = 'open pane normally' - NOT sliding
			bindStartSlidingEvent(pane, false); // remove trigger from resizer-bar & set isSliding=false
		...
		if (s.size > s.maxSize) { // pane is too big! resize it before opening
			s.size = s.maxSize; // update state
			$P.css( c[pane].sizeType, max(1, cssSize(pane, s.size)) ); // update pane size (still hidden though)
		}
		...
		function open_2 () {
			...
			if (slide) { // pane is opening, so BIND trigger events to close it
				if (!s.isSliding) bindStopSlidingEvents(pane, true);
			}
			else // resize all panes adjacent to this one
				sizeMidPanes(c[pane].dir=="vert" ? "center" : "all");

			// set classes and position handles
			setAsOpen(pane);
	...
	var bindStopSlidingEvents = function (pane, enable) {
		...
		if (enable) { // BIND trigger events NEW/FIX
			$R.bind(trigger, slideClosed ); // click OR mouseout - 'click' on resizer will close
			if (trigger == "mouseout") { // NEW/FIX
				$P.bind("mouseout", slideClosed );
				$R.bind("dblclick", close ); // ALWAYS close with dbl-click on Slider-bar
	...
	var slideOpen = function () {
		...
		if (state[pane].isClosed) // skip if already open!
			// NOTE: bindStopSlidingEvents call was moved to open()
			open(pane, true); // true = slide
	};
	...
	var slideClosed = function () {
		...
		,	s = state[pane]
		;
		if (s.isClosed || s.isResizing || c[pane].isMoving)
	...
	var sizePane = function (pane, size) {
		...
		,	$R		= $Rs[pane]
		,	minSize	= calcSize(pane, o.minSize)
		,	maxSize	= calcSize(pane, o.maxSize)
		;
			size	= calcSize(pane, size, true);
		// calculate 'current' min/max sizes
		setPaneMinMaxSizes(pane); // update pane-state
		// compare/update calculated min/max to user-options
		s.minSize = max(s.minSize, minSize);
		if (maxSize > 0) s.maxSize = min(s.maxSize, maxSize);
		// validate passed size
		size = max(size, s.minSize);
		size = min(size, s.maxSize);

		if (size > 0 && size < s.minSize) {
			if (o.paneTooBigAction == "close")
				close(pane, true, true);
			else if (o.paneTooBigAction == "hide")
				hide(pane);
			return;
		}
		...
		if ($R) $R.css( edge, size + cDims[edge] );
	 ...
	 */
	var sizeMidPanes = function (panes, overrideDims, noCallbacks) {
		...
		$.each(panes.split(","), function() {
			...
			if (pane == "center") {
				 ...
				 * ALSO add width to IFRAMES or any other elem that does not'auto'size' correctly
				 */
				if (s.elem != "DIV" || ($.browser.msie && (!$.boxModel || $.browser.version < 7))) {
...
if (false && pane == "center") alert(
	'center.hasRoom = '+ hasRoom +'\n'+
	'center.noRoom = '+ s.noRoom +'\n'+
	'center.isHidden = '+ s.isHidden +'\n'+
	'center.isClosed = '+ s.isClosed +'\n'+
	''
);

				if (s.noRoom) { // CURRENTLY hidden OR closed due to noRoom, so reset it
					s.noRoom = false;
					if (o.noRoomToOpen == "close")
						open(pane); // auto-open pane because was auto-closed
					else if (o.noRoomToOpen == "hide")
						show(pane, !s.isClosed);
				}
				sizeContent(pane);
				if (!noCallbacks) execUserCallback(pane, o.onresize_end || o.onresize);
			}
			else if (!s.noRoom && !s.isHidden) { // no room for pane, so hide it (if not already)
				s.noRoom = true; // update state
				if (o.noRoomToOpen == "hide")
					hide(pane, noCallbacks); // true - noCallbacks
				else if (o.noRoomToOpen == "close")
					close(pane, true, true); // o.noRoomToOpen == "close" (force, noAnimation)
			}
		});
	};
	...
	var sizeContent = function (panes) {
		...
		$.each(panes.split(","), function() {
			...
			$P.children().each(function() {
				var $E = $(this);
				var position = $.curCSS(this, "position");
				if (this == e_C) // Content elem - subtract top/bottom margins */
					height -= (cssNum($E, "marginTop") + cssNum($E, "marginBottom"));
				else if ((!ignore || !$E.is(ignore)) && (position != "absolute" && position != "fixed"))
					height -= $E.outerHeight(true); // NEW: added true to include margins
			});
			if (height > 0)
	...
	var resizeAll = function () {
		...
		if (!cDims.outerHeight) return; // NEW: cannot size layout when 'container' is hidden or collapsed
...
if (false) alert(
	...
	''
		...
		$.each(["south","north","east","west"], function(i,pane) {
			...
			,	s	= state[pane]
			,	dir	= c[pane].dir
			,	chk	= (checkH && dir=="horz") || (checkW && dir=="vert")
			,	$P	= $Ps[pane]
			;
			if ($P && !s.isClosed && (s.autoSize || chk))
				// confirmRoomToOpen will close or hide panes that cannot fit
				if (!chk || confirmRoomToOpen(pane))
					if (s.autoSize) sizePane(pane, o.size); // auto-size pane using original option
		});
	...
	function keyDown (evt) {
		...
		,	s, o, k, m, el
		;
		if (CTRL || SHIFT) // verify that there is a modifier key
			// loop each pane to check its hotkey settings
			$.each(c.borderPanes.split(","), function(i,p) {
				o = options[p];
				k = o.customHotkey;
				// TEST FOR A 'CURSOR HOTKEY'
				m = o.cursorHotkeyModifier; // if missing or invalid, treated as "CTRL+SHIFT"
				if ((SHIFT && m=="SHIFT") || (CTRL && m=="CTRL") || (CTRL && SHIFT)) { // Modifier matches
					if (isCursorKey && options[p].enableCursorHotkey && PANE[code]==p) {// valid cursor-hotkey
						pane = p;
						return false; // BREAK
					}
				}
				// TEST FOR A 'CUSTOM HOTKEY'
 ...
 */
	...
	;
	// NEW: validate the container
	if (!$Container.length) {
		alert('ERROR:  Cannot create a UI/Layout.\n\nThe specified container element does not exist.');
		return {};
	}
