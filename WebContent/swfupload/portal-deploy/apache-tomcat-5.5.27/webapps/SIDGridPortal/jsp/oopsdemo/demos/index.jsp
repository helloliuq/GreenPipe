...
<script type="text/javascript">
	...
	my.gadgetSpecUrls = [
	 webroot+"/SIDGridPortal/gadgets/oops4.xml",
	 webroot+"/SIDGridPortal/gadgets/oops-preview4.xml",
	 webroot+"/SIDGridPortal/gadgets/wfhistory-oops4.xml",
	];
	...
	my.changeSimGagdget = function(){
  	...
  	var result = jsonrpc.AppMgr.showAllScripts("oops");
  	var gadgetList = document.getElementById("gadgetList");
  	for( var i=0; i<result.list.length; i++){
       	var rootUrl = (document.location.href).split("/");
       	var scriptname = result.list[i].scriptName;
       	var scriptfullname;
       	if (result.list[i].version != null )
         		scriptfullname = scriptname+"("+result.list[i].version+")";
      		 else
         		scriptfullname = scriptname;

		var gadgetUrl;
       	if (result.list[i].gadgetLink == null )
         		gadgetUrl = rootUrl[0]+"//"+rootUrl[1]+rootUrl[2]+"/SIDGridGadgets/modpipe/"+scriptfullname+"/"+scriptname+".xml";
       	else
         		gadgetUrl = result.list[i].gadgetLink;
       	console.log(gadgetUrl);
