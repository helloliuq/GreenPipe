...
import org.teragrid.uc.lsgw.services.UserService;
--kavithaa commented */
...
{
    private final static Log log =LogFactory.getLog(MyJSONRPCServlet.class);
	...
	//if(json_bridge.isDebug())
	    ...
	    String decoded_string_1 = OAuth.decodePercent(data.toString());
	    String decoded_string = decoded_string_1.replaceAll("rpc=","");
	//wwj: this is a hack
	    //String wskey = getWsKey(request);
	    //String callstring = decoded_string.replaceAll("unknown", wskey);
	    //log.info("hacked data: "+callstring);
	// Process the request
	JSONObject json_req = null;
	JSONRPCResult json_res = null;
	try {
	    json_req = new JSONObject(decoded_string);
	...
	// Write the response
	//if(json_bridge.isDebug())
	    log.info("send: " + json_res.toString());
